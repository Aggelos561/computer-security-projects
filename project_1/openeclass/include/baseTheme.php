...
if ($is_adminOfCourse and isset($currentCourseID)) {
	if (isset($_GET['hide']) and $_GET['hide'] == 0 and isset($_GET['csrf_token'])) {
	...
	else if (isset($_GET['hide']) and $_GET['hide'] == 1 and isset($_GET['csrf_token'])) {
		if ($_SESSION['csrf_token'] !== $_GET['csrf_token']){
			exit("<h2> Invalid or missing CSRF token </h2>");
		}
			...
			if(file_exists($module_ini_dir = getcwd() . "/module.ini.php")) {
				...
				if (display_activation_link($module_id)) {
					...
					if (visible_module($module_id)) {
						...
						$mod_activation = "<a class='deactivate_module' href='$_SERVER[PHP_SELF]?eclass_module_id=$module_id&amp;hide=0&csrf_token=$token'>($langDeactivate)</a>";
					} else {
						$message = $langActivate;
						$mod_activation = "<a class='activate_module' href='$_SERVER[PHP_SELF]?eclass_module_id=$module_id&amp;hide=1&csrf_token=$token'>($langActivate)</a>";
					}
				}
			}
		}
