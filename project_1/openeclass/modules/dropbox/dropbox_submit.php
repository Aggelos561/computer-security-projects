	...
	if (!$error) {
		...
		if (!$error) {
			...
			$format = get_file_extension($dropbox_filename);
                	$dropbox_filename = substr(hash('sha256', safe_filename()), 0, 20) . "_" . time() . ".$format";
			...
			if (!$error)
			{
				...
				new Dropbox_SentWork($uid, $dropbox_title, htmlspecialchars($_POST['description'], ENT_QUOTES, 'UTF-8'), htmlspecialchars($_POST['authors'], ENT_QUOTES, 'UTF-8'), $dropbox_filename, $dropbox_filesize, $newWorkRecipients);
			}
		}
		chdir ($cwd);
	} //end if(!$error)
