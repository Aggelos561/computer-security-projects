...
if (!isset($_GET['mailing']))  // RH: Mailing detail: no received files
{
	...
	if ($numberDisplayed > 0)
	{
      ...
      <th width='3' style='border: 1px solid #edecdf'>
        <a href='dropbox_submit.php?deleteReceived=all&amp;dropbox_unid=$dr_unid&csrf_token=$token' onClick=\"return confirmationall('".$dropbox_lang['all']."');\"><img src='../../images/delete.gif' title='$langDelete' /></a></th>";
	}
...
tCont9;
	...
	$tool_content .= "
        <a href=\"dropbox_submit.php?deleteReceived=".urlencode($w->id)."&amp;dropbox_unid=".urlencode($dropbox_unid)."&csrf_token=$token\" onClick='return confirmation(\"$w->title\");'>
...
$tool_content .= "
	...
	$tool_content .= "
        <th width='3' style='border: 1px solid #edecdf'>
            <a href='dropbox_submit.php?deleteSent=all&amp;dropbox_unid=".urlencode( $dropbox_unid).$mailingInUrl."&csrf_token=$token'
...
tCont12;
	...
	$tool_content .= "
	<a href=\"dropbox_submit.php?deleteSent=".urlencode($w->id)."&amp;dropbox_unid=".urlencode($dropbox_unid) . $mailingInUrl."&csrf_token=$token\"
