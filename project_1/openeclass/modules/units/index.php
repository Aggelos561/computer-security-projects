...
<script type='text/javascript' src='$urlAppend/include/xinha/my_config.js'></script>";
	...
	if ($id = check_admin_unit_resource($res_id)) {
                @$restitle = htmlspecialchars(@$restitle, ENT_QUOTES, 'UTF-8');
		@$restitle = autoquote(trim(@$restitle));
...
function actions($res_type, $resource_id, $status)
{
	...
	if (!$is_adminOfCourse) {
        ...
        $content .= "<td width='3%'><a href='$_SERVER[PHP_SELF]?del=$resource_id&csrf_token=$token'" .
	...
	if ($status != 'del') {
		if ($res_type == 'text' or $res_type == 'video' or $res_type == 'forum' or $res_type == 'topic') { 
			$content .= "<td width='3%'><a href='$_SERVER[PHP_SELF]?vis=$resource_id&csrf_token=$token'>" .
        ...
        if ($resource_id != $GLOBALS['max_resource_id']) {
                $content .= "<td width='3%'><a href='$_SERVER[PHP_SELF]?down=$resource_id&csrf_token=$token'>" .
                        "<img src='../../template/classic/img/down.gif' title='$langDown'></img></a></td>";
	} else {
		$content .= "<td width='3%'>&nbsp;</td>";
	}
        if (!$first) {
                $content .= "<td width='3%'><a href='$_SERVER[PHP_SELF]?up=$resource_id&csrf_token=$token'>" .
