 ...
 // Display Actions Toolbar
  $tool_content .= "
	  ...
	  <li><a href='newuseradmin.php$linkget'>$linkreg</a></li>
          <li><a href='$_SERVER[PHP_SELF]?show=closed$reqtype&csrf_token=$token'>$langReqHaveClosed</a></li>
...
if (!empty($show) && ($show=="closed")) {
	if (!empty($id) && ($id>0)) {
		...
		while ($req = mysql_fetch_array($sql)) {
			...
			$tool_content .= "<td align=center>
			<a href='$_SERVER[PHP_SELF]?id=$req[rid]&show=closed$reqtype&csrf_token=$token'>$langRestore</a></td>\n  </tr>";
		$k++;
		}
	}
	$tool_content .= "\n  </tbody>\n  </table>\n";
		...
		while ($req = mysql_fetch_array($sql)) {
			...
			$tool_content .= "<td align=\"center\">
				...
				<small>".nice_format(date("Y-m-d", strtotime($req['date_closed'])))."</small></td>";
                	$tool_content .= "<td>".$req['comment']."</td>";
			$tool_content .= "<td align=center>
			<a href='$_SERVER[PHP_SELF]?id=$req[rid]&show=closed$reqtype&csrf_token=$token'>$langRestore</a>
			</td></tr>";
			$k++;
		}
	}
