...
if ($displayForm && (@$addAnnouce==1 || isset($modify))) {
        ...
        $titleToModify = htmlspecialchars($titleToModify, ENT_QUOTES, 'UTF-8');
        $contentToModify = htmlspecialchars($contentToModify, ENT_QUOTES, 'UTF-8');
        $commentToModify = htmlspecialchars($commentToModify, ENT_QUOTES, 'UTF-8');
        
        $titleToModifyEn = htmlspecialchars($titleToModifyEn, ENT_QUOTES, 'UTF-8');
        $contentToModifyEn = htmlspecialchars($contentToModifyEn, ENT_QUOTES, 'UTF-8');
        $commentToModifyEn = htmlspecialchars($commentToModifyEn, ENT_QUOTES, 'UTF-8');
...
if ($displayAnnouncementList == true) {
        ...
        while ($myrow = mysql_fetch_array($result)) {
                ...
                <a href='$_SERVER[PHP_SELF]?delete=$myrow[id]&amp;localize=$localize&csrf_token=$token' onClick='return confirmation();'>
