...
elseif ($a == 1)  {
	if (isset($add)) {
		...
		// OK Create the new faculty
			$codefaculte = htmlspecialchars($codefaculte, ENT_QUOTES, 'UTF-8');
			$faculte = htmlspecialchars($faculte, ENT_QUOTES, 'UTF-8');
...
elseif ($a == 3)  {
	...
	if (isset($_POST['edit'])) {
		validateToken($token);
		// Check for empty fields
                $faculte = htmlspecialchars($_POST['faculte'], ENT_QUOTES, 'UTF-8');
		if (empty($faculte)) {
			$tool_content .= "<p>".$langEmptyFaculte."</p><br>";
