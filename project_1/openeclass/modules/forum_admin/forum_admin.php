...
if(isset($forumgo)) {
	...
	elseif (isset($forumcatsave)) {
		...
		validateToken($token);
		$cat_title = mysql_escape_string(htmlspecialchars($cat_title, ENT_QUOTES, 'UTF-8'));
	...
	elseif(isset($forumgosave)) {
		...
		validateToken($token);
		$forum_name = mysql_escape_string(htmlspecialchars($forum_name, ENT_QUOTES, 'UTF-8'));
		$forum_desc = mysql_escape_string(htmlspecialchars($forum_desc, ENT_QUOTES, 'UTF-8'));
	...
	elseif(isset($forumcatadd)) {
		validateToken($token);
		$catagories = mysql_escape_string(htmlspecialchars($catagories, ENT_QUOTES, 'UTF-8'));
	...
	elseif(isset($forumgodel)) {
		...
		while(list($cat_id, $cat_title) = mysql_fetch_row($result)) {
			...
			<a href='$_SERVER[PHP_SELF]?forumcatdel=yes&cat_id=$cat_id&ok=0&csrf_token=$token' onClick='return confirmation();'>
			<img src='../../template/classic/img/delete.gif' border='0' title='$langDelete'></img></a>
			<a href='$_SERVER[PHP_SELF]?forumcatnotify=$link_notify&cat_id=$cat_id&csrf_token=$token'>	
